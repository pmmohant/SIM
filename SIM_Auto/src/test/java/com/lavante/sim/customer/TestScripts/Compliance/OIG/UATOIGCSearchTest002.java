...
public class UATOIGCSearchTest002 extends PageInitiator {
	...
	/**
	 * Verify the DB Count with UI for OIG for All Status
	 * 
	 * @author Subhas.BV
	 * @param dataMap
	 ...
	 * @throws IOException
	 * 
	 * replacement of OIGPageValidationTC0001
	 */
	@Test(dataProvider = "DBCountOIG", dataProviderClass = SAMOFACOIGDataProvider.class)
	public void OIGCountDBVerify(LinkedHashMap<String, String> dataMap) throws Exception {
		Reporter.log("Test Started for OIG Count DB verify:" + currenttime());

		boolean flag = false;
		String url=driver.getCurrentUrl();
		enOIGdetails.fillSearch(dataMap);
		lavanteUtils.waitForTime(4000);
		lavanteUtils.fluentwait(enOIGdetails.rcrdCount());
	...
	//	if(!url.equalsIgnoreCase(nwurl))
		{
			...
			wait.until(ExpectedConditions.elementToBeClickable(enOIGdetails.rcrdCount()));
		}
		...
		if (dataMap.containsKey("QUERY")) {
			...
			String count = enOIGdetails.rcrdCount().getText();
