...
public class PaymentsPage {
	...
	public List<WebElement> iterateSearchHeaderFindColList(String col) {
		...
		System.out.println("Iterate Search Col: " + col + ":" + j+",Rows:"+ s.size());
		return s;
	}
	 ...
	 * 
	 */
	public void selectPaymentAction(LinkedHashMap<String, String> dataMap) throws FileNotFoundException, IOException, SQLException {
		int index=0;
		lavanteUtils.switchtoFrame(IframeResultsPage);
		lavanteUtils.clicks(ListSelectChkBox.get(index));
		String x=iterateSearchHeaderFindColList("Payment Ref No").get(index).getAttribute("href");
		if(x.contains("paymentId=")){
			String[] y =x.split("paymentId=");
			dataMap.put("PaymentID", y[1].trim());
		}
		dataMap.put("PaymentRefNo", iterateSearchHeaderFindColList("Payment Ref No").get(index).getText());
		dataMap.put("InvoiceNumber", iterateSearchHeaderFindColList("Invoice Number").get(index).getText());
...
private void IterateDownload(LinkedHashMap<String, String> dataMap) {
	...
	private boolean refreshgrid(WebElement iframe,WebElement iframe2, String expStatus) {
		...
		do {
			...
			lavanteUtils.selectbyvalue("11503",Filetype);
			i=i+1;
	...
	public String fetchInv(LinkedHashMap<String, String> dataMap) throws FileNotFoundException, IOException, SQLException {
		String x="";
		String cust="";
		String type="";
		
		if(dataMap.containsKey("CustomerID")){
		 cust=dataMap.get("CustomerID");
		}
		if(dataMap.containsKey("ClaimType")){
			type=dataMap.get("ClaimType");
			
			if(dataMap.containsKey("CustomerID")){

				x = ";WITH CTE AS  "
						+ " ( select MAx(InvoiceNumber) AS InvNUM from InvoiceHeader where CustomerID= "+cust
						+ "  and InvoiceHeaderID in (select InvoiceHeaderID from Payment where   PaymentID not in (Select PaymentID from Claim )) "
						+ " UNION "
						+ " select MAx(InvoiceNumber)AS InvNUM from InvoiceHeader where CustomerID= "+cust
						+ "  and InvoiceHeaderID in (select InvoiceHeaderID from Payment where "
						+ "    PaymentID  in (Select PaymentID from Claim where ClaimTypeID not in "
						+ "    (Select ClaimTypeID from ClaimType where ClaimType='"+type+"')))    ) "
						+ "   SELECT top 1 * FROM CTE";
				x=lavanteUtils.fetchDBdata(x);
			}
		}else if(dataMap.containsKey("CustomerID")){
			 cust=dataMap.get("CustomerID");
			 x=" select MAx(InvoiceNumber) from InvoiceHeader where CustomerID="+cust
						+ "  and InvoiceHeaderID in (select InvoiceHeaderID from Payment where   PaymentID not in (Select PaymentID from Claim ))";
