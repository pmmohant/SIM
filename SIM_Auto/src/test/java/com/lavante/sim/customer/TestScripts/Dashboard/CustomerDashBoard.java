	 ...
	 @Test 
		public void OutstandingTaskCountVerificationTest(){
			...
			if(enobjhomePage.OSTaskIcon().getText().trim().length()>0){
				...
				String x =enobjhomePage.emptyTask().getText();
				Reporter.log("Complete All Task ,Actuals,"+x+"Exp:All tasks are complete",true);
				softAssert.assertEquals(x,"All tasks are complete","All Task message not shown");
				flag=true;
