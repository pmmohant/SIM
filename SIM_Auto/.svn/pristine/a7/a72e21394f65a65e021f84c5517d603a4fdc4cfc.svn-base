...
public class GridReportEmail {
	...
	static String fileSeprator = File.separator;
	static String dir = "QATestData" + File.separator  + "data";
	static String foldername="20180628_103340"; //20180322_080743
	static String htmlfname="C://Users//Piramanayagam.S//Music//"+foldername+"/"+"/html/overview.html";
	//static String htmlfname="D://Grid Result//"+foldername+"/"+"/html/overview.html";
	...
	public static void main() throws InterruptedException, Exception, Throwable
	{
		...
		String file=directory + fileSeprator + dir + fileSeprator + fname;
		File fs = new File(file);
	     ...
	     fontItalic.setItalic(true);
		   ...
		   if(emailCc!=null){
    		...
    		String sub="SIM-Automation Nightly Run,Status:"+date;
    		
    		int browserCount=1;
    		int AllBrowserCount=Integer.parseInt(overallPassed)+Integer.parseInt(overallFailed)+Integer.parseInt(overallSkipped);
    		int AllCount=(Integer.parseInt(overallPassed)+Integer.parseInt(overallFailed)+Integer.parseInt(overallSkipped))/browserCount;
    		...
    		String msg="Hi,\n \t The Test Execution Completed sucessfully at " +date + ", and Grid Report is generated ," +
    				" Publishing the Grid report with the pass rate of  " +overall+ "% .\n \t Summary as follows, \n \t" +
    						...
    						"  2.No of Cross Browser Test     :   "+AllBrowserCount+" \n \t" +
    						"  3.No of Passed Test  	      :   "+overallPassed+" \n \t" +
    						"  4.No of Skipped Test 	  	  :   "+overallSkipped+" \n \t" +
    						"  5.No of Failed Test  		  :   "+overallFailed+" \n \t" +
	...
	@Test
	public static void AfterSuiteCall() throws InterruptedException, Exception, Throwable
	{
		File SuiteFile=new File(lavanteUtils.getPresentWorkingDir()+File.separator+"TestResults");
		SuiteFile=lavanteUtils.recursivefolder(SuiteFile,"overview");
		
		System.out.println(SuiteFile);
		Assert.assertTrue(SuiteFile.isFile(),"Suite Didnot prepare the file");
		...
		String directory = LavanteUtils.getPresentWorkingDir();
		String file=directory + fileSeprator + dir + fileSeprator + fname;
		File gridReportFile = new File(file);
		 HSSFWorkbook workbook = new HSSFWorkbook();
	     ...
	     fontItalic.setItalic(true);
			   ...
			   if(content.contains("<tr class=\"test\">")){
				   ...
				   for(int i=1;i<hsuite.length;i++){
					 ...
					 //      cell.setCellValue(ahptc1[1]);
					       ...
					       try {
					    	   if(!gridReportFile.exists()){
					    		   gridReportFile.createNewFile();
					    	   }
								FileOutputStream out = new FileOutputStream(gridReportFile);
								workbook.write(out);
								out.close();
				  ...
				  System.out.println(gridReportFile);
			   }
		   }
		   ...
		   if(emailCc!=null){
    		...
    		int sdx=2;
    		String[] sf1=new String[sdx];
    		sf1[sdx-sdx]="GridResult.xls";
    		sf1[sdx-1]="Extent.html";
    		
    		File extentFile=new File(directory+fileSeprator+"TestResults");
    		extentFile=lavanteUtils.recursivefolder(extentFile,"Extent");
    		
    		String x=lavanteUtils.getPresentWorkingDir()+fileSeprator+"TestResults";
    		File grdFile=new File(x+fileSeprator+sf1[sdx-sdx]);
    		File extFile=new File(x+fileSeprator+sf1[sdx-1]); 
    		
    		//Extent .html
    		FileUtils.copyFile(extentFile, extFile);
    		FileUtils.copyFile(gridReportFile, grdFile);
    		...
    		file=lavanteUtils.getPresentWorkingDir()+fileSeprator+"TestResults";
			lavanteUtils.sendemail(t,cc,sub,msg,file,sf1);
