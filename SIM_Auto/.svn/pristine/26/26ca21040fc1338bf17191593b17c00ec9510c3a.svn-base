...
public class CustomerPropertySupplierLogin extends PageInitiator {
	...
	public void IS_TAULIA_INVOICETest() throws FileNotFoundException, IOException, SQLException {
		...
		String prop="select CustomerPropertyID from CustomerProperty where PropertyName='IS_TAULIA_INVOICE'";
		prop=lavanteUtils.fetchDBdata(prop);
		String delete = "Delete from CustomerPropertyValue where CustomerID="+customerID+" "
				+ " and CustomerPropertyID in ("+prop+" ) ";
		...
		String insert = "Insert into CustomerPropertyValue ( CustomerPropertyID, CustomerID, PropertyValue, IsActive,CreatedBy, CreatedOn ) VALUES ("+prop+", +customerID+, 'true', 1, 'Admin', getdate())";
