...
package com.lavante.sim.customer.pageobjects.Compliance.SAM;
 ...
 */
public class SAMDetails {

	LavanteUtils lavanteUtils = new LavanteUtils();

	public SAMDetails(WebDriver driver) {
		lavanteUtils.driver = driver;
	}
   
	@FindBy(css="a[href*='customerSAM']")
	private WebElement SAMlink;
	public WebElement SAMlink(){
		return SAMlink;
	}
	...
	@FindBy(css="input[name*='showDeactivatedSuppliers'][type*='checkbox']")
	...
	@FindBy(id="samNote")
	private WebElement samNote;

	public WebElement samNote() {
		return samNote;
	}

	@FindBy(id="saveSAMNote")
	private WebElement saveSAMNote;

	public WebElement saveSAMNote() {
		return saveSAMNote;
	}
	...
	public void formAction(LinkedHashMap<String, String> dataMap) {
		...
		if (ListRadiobtn.size() > 0) {
			...
			if (dataMap.containsKey("Override")) {
				...
				lavanteUtils.typeinEdit(dataMap.get("Override"), samNote);
				lavanteUtils.click(saveSAMNote);
			...
			if (dataMap.containsKey("Deactivate")) {
				...
				lavanteUtils.switchtoFrame(IFRAMEDeactivate);
				lavanteUtils.typeinEdit(dataMap.get("Deactivate"), samNote);
				lavanteUtils.click(saveSAMNote);
				lavanteUtils.switchtoFrame(null);
