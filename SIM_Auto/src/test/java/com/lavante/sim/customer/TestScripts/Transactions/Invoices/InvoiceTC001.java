...
/**
 * Created on 16-12-2015 Enhancement covered SIM -7414,SIM-7526  Ended on 04-02-2016 @author Piramanayagam.S
 * 
...
public class  InvoiceTC001 extends PageInitiator {
	...
	public void EmptySOW05(LinkedHashMap<String, String> dataMap) throws Exception
   {
	 ...
	 */
	@Test
	public void ViewInvoiceAction() throws Exception
	{
		Reporter.log("Test Started for Verification of Status:"+currenttime());
		...
		String appStatus=eninvoicePage.getColumnText("Invoice Status",0);
		Reporter.log("Status for Uploaded Invoice,Verification,Expected:Not Submitted In app,"+appStatus);
		Assert.assertEquals(appStatus, "Not Submitted","Status Not Matched for the Invoice");
		
		String invnum="";
		String expnetAmount="329.2"+randomnum(9);
		outr:if(eninvoicePage.Listinvnum().size()>0){
			for(int i=0;i<eninvoicePage.Listinvnum().size();i++){
				invnum=eninvoicePage.Listinvnum().get(i).getText();
				...
				lavanteUtils.click(eninvoicePage.ViewEdit);
				
				dataMap.put("NetAmnt",expnetAmount);
				dataMap.put("uploadSave","");
				enuploadInvoice.UploadInvoice(dataMap);
		...
		String x=objViewInvoicePage.invoiceHeaderTxt().getText();
		
		Reporter.log("Inv Num: Exp:"+invnum+",Actual:"+x,true);
		softassert.assertTrue(x.contains(invnum),"Inv num not matched");
		
		x=objViewInvoicePage.NetAmntTxt().getText();
		Reporter.log("Net Amount, "+expnetAmount+",Actuals:"+x,true);
		softassert.assertEquals(x,expnetAmount,"Inv num not matched");
		
		softassert.assertAll();
		Reporter.log("Test Ended for Verification of Invoice aCtion Test:"+currenttime());
